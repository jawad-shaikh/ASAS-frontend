    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col p-6 max-w-[450px] rounded-2xl bg-white'>
            ...
            <div className='flex items-center border-b mt-4'>
                <button type='button' onClick={() => setUser("parent")} className={`p-2 font-medium w-full ${user === "parent" ? "border-b-2 border-primary text-primary" : "text-black"}`}>Parent</button>
                <button type='button' onClick={() => setUser("provider")} className={`p-2 font-medium w-full ${user === "provider" ? "border-b-2 border-primary text-primary" : "text-black"}`}>Provider</button>
            </div>
            <div className='mt-3'>
                <FormInput
                    label={"Email Address"}
