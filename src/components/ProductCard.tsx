...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  const { id, thumbnailPicture, title, averageRating, numberOfRatings, ageRangeStart, ageRangeEnd, activityStartDate, activityStartTime, activityEndTime } = product;


  return (
    <Link href={`/user/${id}`} className='block rounded-md shadow-md'>
      ...
      <div className='p-4'>
        ...
        <p className='font-medium text-primary'>{formatDate(activityStartDate)}, {formatTime(activityStartTime)} - {formatTime(activityEndTime)}</p>
      </div>
    </Link>
  );
};
